...
<script>
    @foreach($variaveis as $index => $categorias)
        ...
        new Chart(ctx{{ $index }}, {
            type: 'doughnut', // aqui troca para 'line'
            data: {
                labels: ['Baixo', 'Médio', 'Alto'],
                datasets: [{
                    label: 'Pontuação',
                    data: [{{ $categorias->baixa }}, {{ $categorias->media }}, {{ $categorias->alta }}],
                    fill: false,
                    backgroundColor: ['#0069d9', '#e0a800', '#218838'],
                    borderColor: ['#0069d9', '#e0a800', '#218838'],
                    tension: 0.1
                }]
            },
            options: {
